public class SLLMain {
    public static void main(String[] args) {
        LinkedList11 myLinkedList = new LinkedList11();
        myLinkedList.print();
        myLinkedList.addFirst(800);
        myLinkedList.print();
        myLinkedList.addFirst(700);
        myLinkedList.print();
        myLinkedList.addLast(500);
        myLinkedList.print();
        myLinkedList.insertAfter(700, 300);
        myLinkedList.print();
        myLinkedList.insertAt(3, 250);
        myLinkedList.print();
        myLinkedList.insertAfter(250, 450);
        myLinkedList.print();
        myLinkedList.insertBefore(500, 450);
        

        if (myLinkedList.cekIndex(7)) {
            System.out.println("Data pada index ke-1: " + myLinkedList.getData(7));
        }
        System.out.println("Data 300 berada pada index ke: " + myLinkedList.indexOf(300));

        myLinkedList.remove(300);
        myLinkedList.print();
        myLinkedList.removeFirst();
        myLinkedList.print();
        myLinkedList.removeLast();
        myLinkedList.print();
        myLinkedList.removeAt(0);
        myLinkedList.print();
    }
}
